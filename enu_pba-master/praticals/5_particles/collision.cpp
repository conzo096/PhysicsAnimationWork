...
namespace collision
{
	...
	bool IsColliding(const cSphereCollider &s, const cPlaneCollider &p, dvec3 &pos, dvec3 &norm, double &depth)
	{
		...
		{
			norm = p.normal; pos = sp - norm * distance;
	...
	bool IsColliding(const cCollider &c1, const cCollider &c2, glm::dvec3 &pos, glm::dvec3 &norm, double &depth)
	{
		...
		{
			if (s2 == PLANE)
				return IsColliding(dynamic_cast<const cPlaneCollider &>(c1), dynamic_cast<const cPlaneCollider &>(c2), pos, norm, depth);
			else
				return IsColliding(dynamic_cast<const cPlaneCollider &>(c1), dynamic_cast<const cSphereCollider &>(c2), pos, norm, depth);
		}
		else
		{
			if (s2 == PLANE)
				return IsColliding(dynamic_cast<const cSphereCollider &>(c1), dynamic_cast<const cPlaneCollider &>(c2), pos, norm, depth);
			else
				return IsColliding(dynamic_cast<const cSphereCollider &>(c1), dynamic_cast<const cSphereCollider &>(c2), pos, norm, depth);
		}
	}
}
